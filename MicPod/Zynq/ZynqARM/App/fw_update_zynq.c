...
//checks if the recieved sequence number matches with the internal one (counting)
...
//store recieved FW update binary packet into RAM buffer
...
//MD5 check: fails if computed is different from the recieved one
...
//update FW info and reset RAM region used for binary storing
...
//execute FW update phases: Erasing or writing, and update written pages count
