...
//generate TDM packet from given command
...
//PACKET ON UART:
//looks for packet header and returns the packet len if it identifies it
...
//PACKET ON UART:
//check on packet payload and CRC: fails if CRC doesn't match
...
//PACKET ON TDM:
//checks whole packet integrity: header + payload + CRC
