...
const u32 rx_bus_addr[BUS_NUM]	=	{	XPAR_TDM_HUB_RX_DATA_BRAM_CTRL_S_AXI_BASEADDR};

const u32 tx_bus_addr			=		XPAR_TDM_HUB_TX_DATA_BRAM_CTRL_S_AXI_BASEADDR;

const u32 comm_bus_addr[BUS_NUM]=	{ 	XPAR_TDM_HUB_COMM_MANAGER_BASEADDR};

const u32 bus_intr_id[BUS_NUM]	=	{	XPAR_FABRIC_TDM_HUB_COMM_MANAGER_INTR0_INTR};
...
static long device_ioctl(struct file *filp, unsigned int cmd,unsigned long arg) {
    ...
    switch (cmd) {
		...
		case BUS_GET_LOCK :
		{
			unsigned int locked = (ioread32(&comm[minor][RX_REG_READ])>>RX_LOCKED_SHIFT) &0x01;
			//printk(KERN_INFO "[Bus module] Get locked %d\n",locked);
			put_user(locked,(unsigned int * )arg);
		}
	    default:
	        return -ENOTTY;
    }
