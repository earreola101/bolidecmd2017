...
void cmd_gen_set_loopback_en(uint8_t receiver, uint8_t sender,uint8_t en, tdm_command_t * command)
{
    command->command=CMD_SET_LOOPBACK_MICPOD;
    command->len=1;
    command->sender=sender;
    command->receiver=receiver;
    command->sequence=0;
...
void cmd_dec_set_loopback_en(uint8_t  *en, tdm_command_t  *command)
{
    *en = command->data[0];
}
