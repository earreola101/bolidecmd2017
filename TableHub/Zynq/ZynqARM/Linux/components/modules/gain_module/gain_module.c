...
#define DEVICE_NAME "gain_module"

// imported from xparameters.h



#define REG_BASE_ADDRESS		XPAR_AUDIO_GAIN_BLOCK_BASEADDR
...
static ssize_t device_write(struct file *filp,const char __user *buffer,size_t len, loff_t *off) {
	unsigned int dataW;
	copy_from_user(&dataW,buffer,4);
	//printk(KERN_INFO"Writing gain %d\n",dataW);
	iowrite32(dataW, regBaseAddrPtr);
	return 0;
}
