...
#define MEM_BASE_ADDRESS 		XPAR_AUDIO_BRAM_CTRL_TO_PS_S_AXI_BASEADDR
#define REG_BASE_ADDRESS		XPAR_AUDIO_AUDIO_TO_PS_BASEADDR
#define AUDIO_PS_IRQ 			(XPAR_FABRIC_AUDIO_AUDIO_TO_PS_INTR_INTR)

#define AUDIO_TO_PS_MS_PER_BUF_MAX		10
...
irqreturn_t audioToPsIntHandler(int irq, void *dev_id);


static int audio_to_ps_flag = 0; 
static int audio_to_ps_position = 0;

static int running=0;



void audioToPsTasklet(unsigned long unused);
DECLARE_TASKLET_DISABLED(audioToPsDeviceTasklet,audioToPsTasklet, 0);
...
void audioToPsTasklet(unsigned long unused) {
	...
	iowrite32(AUDIO_TO_PS_ON, &regBaseAddrPtr[1]);
	audio_to_ps_position = (ioread32(&regBaseAddrPtr[0])>>1)&0x01;
    audio_to_ps_flag = 1;
...
{
	//clear interrupt
	iowrite32(AUDIO_TO_PS_CLR_ON, &regBaseAddrPtr[1]);
    tasklet_schedule(&audioToPsDeviceTasklet);
    return IRQ_HANDLED;
}

static long device_ioctl(struct file *filp, unsigned int cmd,unsigned long arg) {
	int result = SUCCESS;
    if (_IOC_TYPE(cmd) != AUDIO_TO_PS_IOC_MAGIC) return -ENOTTY;
	if (_IOC_NR(cmd) > AUDIO_TO_PS_IOC_MAXNR) return -ENOTTY;
    switch (cmd) {
        case AUDIO_TO_PS_SET_BUF_LEN :
	    {
			
			if(arg>AUDIO_TO_PS_MEM_SIZE)
			{	
				printk(KERN_NOTICE"[audio_to_ps] size out of range: try %d, max allowed %d\n",arg,AUDIO_TO_PS_MEM_SIZE);
				return -EINVAL;
			}
			if(running==0)
			{
				audio_to_ps_size = arg;
				audio_to_ps_half_size = arg/2;
				audio_to_ps_sample_per_buffer = arg/8;
				iowrite32(AUDIO_TO_PS_OFF, &regBaseAddrPtr[1]);
				printk(KERN_NOTICE"[audio_to_ps] size changed to %d\n",audio_to_ps_size);
			}
			else
			{
				printk(KERN_NOTICE"[audio_to_ps] running: can't change buffer size when running\n");
			}
			return SUCCESS;
	    }
	    case AUDIO_TO_PS_SET_ON_OFF :
	    {
			if(arg==1)
			{
				if(running==1)
				{
					printk(KERN_NOTICE"[audio_to_ps] aready running with size %d\n",audio_to_ps_size);
				}
				else
				{
					iowrite32(AUDIO_TO_PS_ON, &regBaseAddrPtr[1]);

					tasklet_enable(&audioToPsDeviceTasklet);
					result = request_irq(AUDIO_PS_IRQ, audioToPsIntHandler,0, DEVICE_NAME, NULL);
					if (result) {
						printk(KERN_INFO "[audio_to_ps] %s: can't get irq assigned, error %d\n",DEVICE_NAME, result);
						deviceOpenDesc = 0;
						return -EIO;
					}

					printk(KERN_NOTICE"[audio_to_ps] started with size %d and irq num %d\n",audio_to_ps_size,AUDIO_PS_IRQ);
			...
			else
			{
				if(running==0)
				{
					printk(KERN_NOTICE"[audio_to_ps] audio ps aready stopped\n");
				}
				else
				{
					iowrite32(AUDIO_TO_PS_OFF, &regBaseAddrPtr[1]);

					free_irq(AUDIO_PS_IRQ, NULL);
					tasklet_disable(&audioToPsDeviceTasklet);
					
					printk(KERN_NOTICE"[audio_to_ps] audio ps stopped\n");
					running=0;
				}
			}
	        return SUCCESS;
...
static ssize_t device_read(struct file *filp, char __user *buffer,size_t len, loff_t *off) {
	...
	if(len>audio_to_ps_half_size)
	{
		printk(KERN_ALERT"[audio_to_ps] Audio read invalid length: got %d while max is %d\n", len, audio_to_ps_half_size);
		return -EINVAL;
	}	
	//printk(KERN_INFO "Read going to sleep\n");
    wait_event_interruptible(queue, audio_to_ps_flag != 0);
    audio_to_ps_flag = 0;
    //printk(KERN_INFO "Read awoken\n");
	
	if(audio_to_ps_position)
		pos=0;
	else
		pos=1;
	for(i=0; i<len/4; i++)	
	{
		dataR[i] = ioread32(&memBaseAddrPtr[i+pos*audio_to_ps_sample_per_buffer]);	
...
static int device_release(struct inode *inode, struct file *file) {
	...
	if(running ==1)
	{
		...
		free_irq(AUDIO_PS_IRQ, NULL);
		tasklet_disable(&audioToPsIntHandler);

		printk(KERN_NOTICE"[audio_to_ps] audio ps stopped by close\n");
    }
	running =0;
...
static int device_open(struct inode *inode, struct file *file) {
	...
	if(ioread32(&regBaseAddrPtr[0])&0x1)
	{
		iowrite32(AUDIO_TO_PS_CLR_OFF, &regBaseAddrPtr[1]);
	}
	iowrite32(AUDIO_TO_PS_OFF, &regBaseAddrPtr[1]);
	running =0;
	
	printk(KERN_INFO"[audio_to_ps] Open\n");
	

	dataR = 0;
	dataR = kmalloc(audio_to_ps_half_size,GFP_KERNEL);
	if(dataR == 0)
		printk(KERN_WARNING"kmallok error\n");
...
int bdInitModule(void) {
    ...
    if(result < 0){
        printk(KERN_WARNING "[audio_to_ps] %s: can't get device number: %d",DEVICE_NAME, result);
        return result;
    }
 	major_n=MAJOR(devNo);
	printk(KERN_INFO "[audio_to_ps] %s: 'mknod /dev/%s c %d 0'.\n",DEVICE_NAME,DEVICE_NAME, major_n);
