...
#define DEVICE_NAME "version_module"

// imported from xparameters.h



#define REG_BASE_ADDRESS		XPAR_VERSION_IP_BASEADDR
...
static ssize_t device_read(struct file *filp, char __user *buffer,size_t len, loff_t *off) {
	...
	int pos;	
	uint32_t dataR[3];
	uint32_t *ubuffer = (unsigned int *)buffer;
	for(i=0; i<3; i++)	
	{
		dataR[i] = ioread32(&regBaseAddrPtr[i]);	
		//printk(KERN_NOTICE"Data %d 0x%x\n", i, dataR[i]);
		put_user(dataR[i], &ubuffer[i]);
	}
;
	return 12;
}
